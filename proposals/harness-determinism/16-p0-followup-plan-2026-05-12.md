...
8. spike 종료 — debug branch revert, env unset, dump 파일 삭제. verify: `grep -r SAZO_PROMPT_SPIKE_DUMP packages/ai-harness/scripts/hooks/` empty (GNU grep는 디렉토리에 `-r`/`--directories=recurse` 필요. 없으면 `Is a directory` 에러).
