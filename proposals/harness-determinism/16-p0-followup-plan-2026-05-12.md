...
4. user global `~/.claude/CLAUDE.md` managed block source 위치 grep — `packages/ai-harness/install.sh` 내 managed block 정의. 그 파일의 "구현" 표 row에 1줄 경고 inline. verify: 다음 install 후 user CLAUDE.md에 경고 반영.
...
4. **PR 4**: F3 문서화 (CLAUDE.md + agent + workflow-hooks.md).
