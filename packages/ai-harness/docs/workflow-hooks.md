...
**Alpha**. 활성화 = dogfood 의지 표현. False positive 발생 시 issue 등록 부탁.
다음 항목 정리되기 전엔 default-on 안 됨:
...
| `pre-exploration-gate.sh` | PreToolUse | `Grep\|Bash` | Opus 직접 탐색 block |
| `workflow-state-machine.sh pre` | PreToolUse | `Write\|Edit\|NotebookEdit\|Bash` | research/plan/approval/ci/review gate |
| `workflow-state-machine.sh post` | PostToolUse | `Task\|Bash` | stage 자동 완료 + explore_count decay |
...
각 hook 독립 실행. 하나 실패해도 나머지 작동. opt-in 기본이라 도입 충격 최소화.
