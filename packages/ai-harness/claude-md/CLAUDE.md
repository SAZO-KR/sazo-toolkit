...
- 이유: subagent는 독립 컨텍스트에서 자기 모델(haiku/sonnet)로 실행되어, 메인(Opus) 토큰을 소비하지 않는다. 파일을 수십 개 직접 grep/read 하면 전부 메인 컨텍스트에 쌓인다.
...
| 6. 코드리뷰 | `nori-code-reviewer` + `oracle` (심층) | opus |
...
- `nori-code-reviewer` subagent로 기본 리뷰, 아키텍처/심층 판단이 필요하면 `oracle` 추가. 두 subagent는 **병렬 호출**.
